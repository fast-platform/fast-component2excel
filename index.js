...
function testImport() {
  fs.readFile('./w_testData/expWorkBook.xlsx', null, function(err, data) {
    ...
    workBookPromise.then((workBook) => {
      ...
      Main.saveJsonToFile(
        formDefFixed,
        './w_result/imp' + 'FormDefFixed' + '.json',
      );
      ...
      Main.saveJsonToFile(
        formDefWithVal,
        './w_result/imp' + 'FormDefWithVal' + '.json',
      );
...
function testExport() {
  fs.readFile('./w_testData/formDef.json', 'utf8', function(err, data) {
    ...
    Main.saveJsonToFile(
      formDefFixed,
      './w_result/exp' + 'FormDefFixed' + '.json',
    );
    //console.log(formDefFixed);
    ...
    Main.saveJsonToFile(
      mainSheetArr,
      './w_result/exp' + 'MainSheetArr' + '.json',
    );
    //console.log(mainSheetArr);

    // + 'formDefFixed' sheet
    let workBookPromise = Main.getExpWorkBookPromise(
      mainSheetArr,
      formDefFixed,
    );
